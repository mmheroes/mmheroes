...
fn display_initial_parameters(r: &mut impl Renderer, mode: GameMode) -> Action {
    ...
    r.set_color(Color::White);
    r.write_str("Случайный студент\n");
    r.write_str("Шибко умный\n");
    r.write_str("Шибко наглый\n");
    r.write_str("Шибко общительный\n");
    if mode == GameMode::God {
        r.write_str("GOD-режим");
    }
    wait_for_any_key(r);
    loop {}
    todo!()
