...
fn regenerate_ios_files() -> Result<(), Error> {
    ...
    let mut main_manifest_file = std::fs::OpenOptions::new()
        .read(true)
        .open("Cargo.toml")?;

    let mut output = String::new();

    writeln!(output, "# Warning! This file has been autogenerated by the build script.")?;
    writeln!(output, "# Do not edit it manually.")?;
    writeln!(output)?;
    writeln!(output, "# This file is the same as the main Cargo.toml, but it sets the crate type")?;
    writeln!(output, "# to be staticlib so it can be used by iOS clients.")?;
    writeln!(output)?;
    ...
    writeln!(output)?;
    writeln!(output, "# Turn off unwinding so we get proper stack trace in Xcode.")?;
